...
** DISCUSSION
The Meanshift algorithm produces a variable number of clusterings based on the runtime. 13/14 seem to be the most common ones based on our runs. On the other hand, the agglomerative clustering allows us to select the number of clusters we like. We tried various numbers between 3-10 and stuck with 4 a bit arbitrarily. We visualize the results by plotting the data as a scatterplot. As the scatter plot is 2d, we have to dimension reduce our data, which means that the clustering may not necessarily be as easily differentiated as it might be on a higher-dimension scatter plot.

Based on the tests and the scatter plots, we do see that the data is clustered to some degree. For example, all yellow dots are either top right or at the top. Green tend to be top and middle left, while blue is bottom right on all the agglomerative clustering.

While the clustering is different in the meanshift, we can still see the conglomeration of points fo same color together lke yellow on the right, purple on the left/middle, etc...
...
with open('encoded_chunks.pkl', 'rb') as f:
...
for linkage, metric in linkage_metrics:
    ...
    #Added dimension reduction for scatter plot view
    ...
    plt.scatter(
        ...
        c=label, s=25, alpha=0.8
    )
    plt.title(f"Agglomerative Clustering ({linkage} - {metric})")
    plt.xlabel("DIM 1")
    plt.ylabel("DIM 2")
    plt.colorbar(label="Cluster")

    plt.savefig(f"{linkage}-{metric}.png")
    plt.close()
...
** Agglomerative clustering scatterplot (PCA for dimension reduction)
[[./ward-euclidean.png]]

[[./average-manhattan.png]]
...
# Changes the data so the numbers are all on one same scale, makes it easier to work with as we continue
scaler = StandardScaler()
scaledData = scaler.fit_transform(data)

# Gets a good bandwidth to work with for the clustering
bandwidthChosen = estimate_bandwidth(scaledData, quantile=0.1, n_samples=500)
print(f"Estimated bandwidth: {bandwidthChosen}")

# Perform MeanShift clustering
clustersMade = MeanShift(bandwidth=bandwidthChosen)
graphLabels = clustersMade.fit_predict(scaledData)

# Gets the number of clusters
numOfClusters = len(np.unique(graphLabels))
print(f"MeanShift produced {numOfClusters} clusters")

# Gets a score to see if the clusters are good
if numOfClusters > 1:
    silhouette = silhouette_score(scaledData, graphLabels)
    print(f"Silhouette Score: {silhouette:.2f}")
else:
    print("Silhouette score cannot be calculated because only one cluster was found.")

# Transforms the data to allow a picture to be made
reducer = PCA(n_components=2)
pca_data = reducer.fit_transform(scaledData)

# Clusters are plotted and output to a png
plt.figure(figsize=(6, 6))
plt.scatter(
    pca_data[:, 0], pca_data[:, 1],
    c=graphLabels, s=25, alpha=0.8
)
plt.title(f"MeanShift Clustering (Clusters: {numOfClusters})")
plt.xlabel("PCA Dimension 1")
plt.ylabel("PCA Dimension 2")
plt.colorbar(label="Cluster")

# Saves picture for later use
...
#+RESULTS:
: Estimated bandwidth: 1.1592555239714966
: MeanShift produced 13 clusters
: Silhouette Score: 0.41

** MeanShift clustering scatterplot 
[[./MeanShift Clustering.png]]
