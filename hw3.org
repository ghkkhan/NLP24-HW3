...
# Load Brown corpus (category news) and remove stop words
stop_words = set(stopwords.words("english"))
news_sentences = brown.sents(categories='news')

# Helper function to create bag of significant words
...
for i in first_anchors:
    ...
    baseline_intersections.append(len(anchor_bow.intersection(successor_bow)))
...
shuffled_sentences = shuffle(news_sentences, random_state=42)
experiment_intersections = []
for i in second_anchors:
    anchor_bow = significant_words(shuffled_sentences[i])
    successor_bow = significant_words(shuffled_sentences[i + 1])
    experiment_intersections.append(len(anchor_bow.intersection(successor_bow)))

# Displaying Results
print("(first anchor + successors):", Counter(baseline_intersections))
print("(second anchor + successors):", Counter(control_intersections))
print("(second anchor + new successors):", Counter(experiment_intersections))
